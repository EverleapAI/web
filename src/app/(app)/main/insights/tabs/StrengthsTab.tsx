...
function pillButton(dark: boolean) {
  return [
    "inline-flex items-center justify-center gap-2 rounded-full border px-3.5 py-2 text-xs font-semibold",
    dark ? "border-white/10 bg-white/6 text-white/78" : "border-black/10 bg-white text-slate-900",
  ].join(" ");
}
  ...
  return (
    <span
      className={[
        ...
        dark ? "bg-fuchsia-300/75" : "bg-fuchsia-500/80",
      ].join(" ")}
    />
  );
}
...
export function StrengthsTab(props: {
  ...
  const patterns = strengths.strengths ?? [];
  const inYourElement = strengths.inYourElement ?? [];
  const hardMode = strengths.hardMode ?? [];

  const experiment = strengths.experiment ?? {};
  const experimentTitle = experiment.title ?? "One small experiment";
  const experimentSteps = experiment.steps ?? [];

  const unlock =
    asUnlock(strengths.primaryUnlock) ??
    ...
    <section className="mb-6">
      <div className="relative">
        <div className={readingSurface(dark)}>
          <div className="relative">
            <div className={`text-[24px] md:text-[28px] ${sectionTitle(dark)}`}>
              {headline}
            </div>

            {/* story */}
            <div className="mt-3">
              <p className={bodyText(dark)}>
                {storyExpanded ? storyTextExpanded : storyTextCollapsed}
              </p>
            </div>
