...
import { ArrowRight, CircleHelp, Globe } from "lucide-react";
...
function buildAgenticSummary(experience: WorldExperience) {
  ...
  if (description) {
    if (/^a path for people who/i.test(description)) {
      return ensureSentence(
        description.replace(/^a path for people who\s*/i, "This world could fit if you ")
      );
    }

    if (/^for people who/i.test(description)) {
      return ensureSentence(
        description.replace(/^for people who\s*/i, "This world could fit if you ")
      );
    }
  ...
  return "This world could be worth a closer look if the places, people, and kinds of experiences inside it already feel alive to you.";
}
          ...
          <div className="relative flex items-start justify-between gap-4 lg:gap-5">
            <div className="min-w-0">
              ...
              <p className="mt-2 max-w-2xl text-[13px] leading-[1.65] text-white/72 lg:text-[14px]">
                See what this world feels like from the inside, why it may fit,
                and where its opportunities can lead.
              </p>
            </div>
