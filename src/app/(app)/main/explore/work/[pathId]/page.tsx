        ...
        >
          ...
          <div className="relative grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
            <div className="min-w-0">
              ...
              <p className={`mt-5 max-w-3xl text-sm leading-7 sm:text-[15px] ${textSoft(dark)}`}>
                {path.hero.summary}
              </p>
          ...
          >
            ...
            <div className="mt-4 space-y-4">
              {path.fitSignals.map((signal) => (
                <div key={signal.id} className="rounded-[22px] border border-white/8 bg-white/[0.04] px-4 py-4">
                  <div className="flex items-center justify-between gap-3">
                    <div className={`text-sm font-semibold ${textMain(dark)}`}>{signal.label}</div>
                    <div className="text-xs font-semibold text-white/58">{signal.score}</div>
                  </div>
                  ...
                  <div className="mt-3 h-2 overflow-hidden rounded-full bg-white/8">
                    <div
                      ...
                      style={{
                        ...
                        background: `linear-gradient(90deg, ${rgb(path.theme.accent, 0.95)} 0%, ${rgb(
                  ...
                  <p className={`mt-3 text-sm leading-6 ${textSoft(dark)}`}>{signal.explanation}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="mt-4 grid gap-3">
              {path.snapshotStats.map((stat) => (
                ...
                >
                  ...
                  <div className={`mt-2 text-base font-semibold ${textMain(dark)}`}>{stat.value}</div>
                  {stat.note ? (
        ...
        >
          <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 className={`mt-2 text-2xl font-semibold tracking-tight ${textMain(dark)}`}>
                Where this path can branch
              </h2>
              <p className={`mt-2 max-w-2xl text-sm leading-6 ${textSoft(dark)}`}>
                Different versions of this work emphasize different strengths. Some people light up around systems, some around space and pacing, and some around story and player emotion.
              </p>
            </div>
  ...
  {specialty.energy ? (
    <span className="rounded-full border border-white/10 bg-white/[0.05] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-white/56">
      {specialty.energy.replace("-", " ")}
                ...
                <div className={`mt-2 text-sm leading-6 ${textSoft(dark)}`}>{specialty.oneLiner}</div>
                <div className={`mt-3 text-sm leading-6 ${textFaint(dark)}`}>{specialty.whyItCouldFit}</div>
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          ...
          >
            ...
            <div className={`mt-2 text-xl font-semibold ${textMain(dark)}`}>{path.dayInLife.title}</div>
            <div className={`mt-2 text-sm leading-6 ${textSoft(dark)}`}>{path.dayInLife.summary}</div>
          ...
          >
            ...
            <div className={`mt-2 text-xl font-semibold ${textMain(dark)}`}>{path.forecast.title}</div>
            <div className={`mt-2 text-sm leading-6 ${textSoft(dark)}`}>{path.forecast.summary}</div>
            <div className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-white/78 group-hover:text-white">
              Open page
              <ArrowRight className="h-4 w-4" />
            </div>
          ...
          >
            ...
            <div className={`mt-2 text-xl font-semibold ${textMain(dark)}`}>{path.nextSteps.title}</div>
            <div className={`mt-2 text-sm leading-6 ${textSoft(dark)}`}>{path.nextSteps.summary}</div>
            <div className="mt-4 inline-flex items-center gap-2 text-sm font-semibold text-white/78 group-hover:text-white">
              Open page
              <ArrowRight className="h-4 w-4" />
            </div>
