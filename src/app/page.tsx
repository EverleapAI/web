...
export default function HomePage() {
  ...
  React.useEffect(() => {
    ...
    fetch("/api/regauth/me", {
      ...
      .then((data) => {
        if (!alive) return;
        setAuthed(Boolean(data?.ok && data?.authed && data?.user));
      })
      .catch(() => {
        if (!alive) return;
        setAuthed(false);
  ...
  const effectiveObjectPosition = isDesktop ? "50% 50%" : "18% 52%";
  ...
  const startFadeTo = React.useCallback(
    (next: Phase) => {
      if (videoError || fadingRef.current) return;
    ...
    [phase, safePause, safePlay, safeReset, videoError]
  );
  ...
  const handleTimeUpdate = React.useCallback(
    (who: Phase) => {
      if (videoError || fadingRef.current || phase !== who) return;
  ...
  const boyOpacity = anyReady && phase === "boy" ? "opacity-100" : "opacity-0";
  const girlOpacity = anyReady && phase === "girl" ? "opacity-100" : "opacity-0";
  ...
  if (authed === null) {
    return (
      <div className="w-full rounded-3xl border border-white/10 bg-slate-950/58 px-6 py-8 text-center shadow-[0_18px_70px_rgba(0,0,0,0.72)] backdrop-blur-lg md:px-8 md:py-11">
        ...
        <div className="mt-7 h-[92px] md:mt-8" />
      </div>
    );
  }

  const ctaHref = authed ? "/main" : "/onboarding";
  const ctaLabel = authed ? "Continue your path" : "Let's get started";
    ...
    <div className="w-full rounded-3xl border border-white/10 bg-slate-950/58 px-6 py-8 text-center shadow-[0_18px_70px_rgba(0,0,0,0.72)] backdrop-blur-lg md:px-8 md:py-11">
      ...
      <div className="mt-7 flex flex-col items-center justify-center gap-3 md:mt-8">
        <Link
          ...
          onClick={() => {
            if (!authed) {
              localStorage.removeItem("everleap_onboarding_answers");
              localStorage.removeItem("everleap_ai_lab_answers");
            }
          }}
