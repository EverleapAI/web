...
const VERIFY_EMAIL_URL =
  `${API_BASE}/auth/email/verify-code`;
...
function getSecret(): string {
  ...
  if (!secret) {
    throw new Error(
      "Missing env REGAUTH_COOKIE_SECRET"
    );
  }
...
function getUpstreamSetCookies(
  upstream: Response
): string[] {
...
function extractCookieValue(
  ...
  const escapedName = cookieName.replace(
    /[.*+?^${}()|[\]\\]/g,
    "\\$&"
  );
...
export async function POST(
  ...
  const pendingToken =
    req.cookies.get(PENDING_COOKIE)?.value;
  ...
  try {
    pending = decodeSigned<PendingPayload>(
      getSecret(),
      pendingToken
    );
  } catch {
    return jsonError(
      "Server not configured for sign-in.",
      500
    );
  }
  ...
  const allSetCookies =
    getUpstreamSetCookies(upstream);

  const upstreamSessionCookie =
    allSetCookies.find((cookie) =>
      cookie.startsWith(
        `${SESSION_COOKIE}=`
      )
    );
